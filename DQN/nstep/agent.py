...
# experience replay
class Nstep_ReplayBuffer(object):
    def __init__(self, capacity, n_multi_step = 3, gamma = 0.99):
        self.buffer = deque(maxlen=capacity)
        self.n_multi_step = n_multi_step
        self.gamma = gamma

    def push(self, state, action, reward, next_state, done): #存还是存next_state 这个transitions，但是在sample时会变化
        self.buffer.append((state, action, reward, next_state, done))

    # 有关这个采样过程，sample的index和参照代码是不同的
    def sample(self, batch_size):

        global state_look_ahead, done_look_ahead
        len_sample = min(len(self.buffer), batch_size)
        batch_state, batch_action, batch_reward, batch_next_state, batch_done = [], [], [], [], []
        for i in range(len_sample):
            begin = random.randrange(len(self))
            sum_reward = 0
            for j in range(self.n_multi_step):
                sum_reward = self.buffer[begin + i][2] * (self.gamma ** j)
                state_look_ahead = self.buffer[begin + i][3]
                done_look_ahead = self.buffer[begin + i][4]
                if ~done_look_ahead:
                    break
            batch_state.append(self.buffer[0])
            batch_action.append(self.buffer[1])
            batch_reward.append(sum_reward)
            batch_next_state.append(state_look_ahead)
            batch_done.append(done_look_ahead)
        # # array转为tensor
...
class Agent:
    def __init__(self, num_state, num_action):
        ...
        self.memo = Nstep_ReplayBuffer(1000,n_multi_step = 4, gamma = self.gamma)
