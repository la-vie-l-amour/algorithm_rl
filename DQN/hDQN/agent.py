...
class Agent:
    def __init__(self, num_state, num_action):
        ...
        # why input's dimension is 2 multiply num_state
        self.online_net = DQN(num_state= 2 * num_state,num_action=num_action).to(self.device)
        self.target_net = DQN(num_state= 2 * num_state, num_action=num_action).to(self.device)
        self.memo = ReplayBuffer(1000)
        self.optimizer = torch.optim.Adam(self.online_net.parameters(), lr= self.learning_rate)

        self.meta_online_net =  DQN(num_state= num_state,num_action=num_action).to(self.device)
        self.meta_target_net =  DQN(num_state= num_state,num_action=num_action).to(self.device)
        self.meta_memo = ReplayBuffer(1000)
        self.meta_optimizer = torch.optim.Adam(self.meta_online_net.parameters(), lr= self.learning_rate)



    def act(self, state, epsilon, online_net):
        if random.random() > epsilon:
            state = torch.as_tensor(state, dtype=torch.float32).unsqueeze(0)# [1,2,3,4]->[[1,2,3,4]]
            state = state.to(self.device)
            q_value = online_net(state)  # [[1,2]]这种形式
    ...
    def compute_td_loss(self, online_net, target_net, optimizer, memo):
        # learn until memo has enough transitions ,at least > batch_size
        if self.batch_size > len(memo):
            return
        batch_state, batch_action, batch_reward, batch_next_state, batch_done = memo.sample(self.batch_size)
        ...
        next_q_values_online = online_net(batch_next_state)
        ...
        q_values = online_net(batch_state)
